...
###
### Ellyn Butler
### October 11, 2019 - December 17, 2019
...
for (varlist in variances) {
  for (distr in distrs) {
    ...
    print("May want to add distribution info back into plot")

    assign(paste0("corrplot_", distr[1], "_", varlist[[1]][1], "_", varlist[[2]][1]), corr_plot)
    write.csv(corr_df, paste0("/Users/butellyn/Documents/brainAgeGapMistake/data/corr_oneBrain_", distr[1], "_", varlist[[1]][1], "_", varlist[[2]][1], ".csv"), row.names=FALSE)
  }
}

pdf(file="/Users/butellyn/Documents/brainAgeGapMistake/plots/functionAndSimulation_one.pdf", width=2.5, height=3)
#corrplot_normal_2_4
corr_plot
...
corr_plot <- ggplot(corr_df, aes(x=CorrBrainAge, y=CorrBadPredAgeAge)) +
  geom_abline(aes(colour="identity line"), intercept=0, slope=1, colour="grey70") +
  geom_line(aes(y=CorrBadPredAgeAge_Function, colour="r_func")) + ylab("Inflated Correlation") +
  geom_point(aes(colour="r_trans"), shape=18, color="blue", alpha=.5, size=1.5) + theme_bw() + xlab("True Correlation") +
  scale_y_continuous(limits=c(0, 1), breaks=seq(0, 1, .2)) +
  scale_x_continuous(limits=c(0, 1), breaks=seq(0, 1, .2)) +
...
write.csv(multivariate_table, "/Users/butellyn/Documents/brainAgeGapMistake/data/multivariate_table.csv", row.names=FALSE)
